package com.bocai.select.photo.adapter;
...
import com.bocai.select.photo.R;
import com.bocai.select.photo.entry.Image;
...
public class ImageAdapter extends RecyclerView.Adapter<ImageAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_IMAGE) {
            View view = mInflater.inflate(R.layout.photo_adapter_images_item, parent, false);
            return new ViewHolder(view);
        } else {
            View view = mInflater.inflate(R.layout.photo_adapter_camera, parent, false);
            return new ViewHolder(view);
        }
    }
    ...
    private void checkedImage(ViewHolder holder, Image image) {
        ...
        if(mMaxCount == 9 && selectedCount >= 9){
            Toast.makeText(mContext,"最多可以选择9张图片",Toast.LENGTH_SHORT).show();
        }
        selectedCount = mSelectImages.size();
    }
    ...
    private void setItemSelect(ViewHolder holder, boolean isSelect) {
        if (isSelect) {
            holder.ivSelectIcon.setImageResource(R.drawable.photo_icon_image_select);
            holder.ivMasking.setAlpha(0.5f);
        } else {
            holder.ivSelectIcon.setImageResource(R.drawable.photo_icon_image_un_select);
